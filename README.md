...
The current prediction architecture uses **382,612 trainable parameters**.
...
Produces the prediction table at the path supplied to `--output`. Additional
split tables and VCFs are written beside it using the same output stem; for
example, `--output somatix_predict.txt` also writes `somatix_predict.somatic.txt`,
`somatix_predict.germline.txt`, `somatix_predict.vcf`,
`somatix_predict.somatic.vcf` and `somatix_predict.germline.vcf`.
...
- Main prediction table: path supplied to `--output`
- Split tables: `<output-stem>.somatic.txt`, `<output-stem>.germline.txt`
- VCFs: `<output-stem>.vcf`, `<output-stem>.somatic.vcf`, `<output-stem>.germline.vcf`
