...
## Example: small-region test (quick sanity check)

```bash
# extract short-region candidates
somatix candidates \
  --bam sample_case.bam --ref ref.fa --output case_cand.txt --region chr1:100000-200000 --allele-counter ./allele_counter

# extract case features for that region
somatix features \
  --candidates case_cand.txt --bam sample_case.bam --ref ref.fa \
  --output-prefix case_features --filtered-prefix case_cand.filtered --region chr1:100000-200000 --threads 4

# extract control features at the same candidate sites
somatix features \
  --candidates case_cand.txt --bam sample_control.bam --ref ref.fa \
  --output-prefix control_features --filtered-prefix case_cand.filtered --region chr1:100000-200000 --threads 4

# predict
somatix predict \
  --case-features-prefix case_features --control-features-prefix control_features \
  --variant-prefix case_cand.filtered --model somatix_model.pth --output test_predict.txt  --region chr1:100000-200000
```
